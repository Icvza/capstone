...
export default function PlannerPage({ place }: any) {
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (searchInput === "") {
      ...
      const res = await fetch(
        `http://localhost:3000/api/filter/${searchInput}`
      );
      const filteredPlaces = await res.json();
  ...
  const iconSort = async ({ iconFilterValue }: any) => {
    ...
    const res = await fetch(
      `http://localhost:3000/api/filter/${iconFilterValue}`
    );
    const filteredPlaces = await res.json();
    ...
    <div>
      ...
      <div className="p-8 pb-12 pt-56 bg-cover w-full h-fit bg-[url('../public/benjamin-lizardo-d-OoxMOAIgU-unsplash.jpg')]">
        ...
        <div className="flex flex-col justify-center items-center h-40 p-4 pl-12 pr-12 sm:items-center sm:flex-col sm:justify-center md:flex-col md:justify-center lg:justify-between lg:flex-row">
          {/* FORM */}
          <div className="flex flex-row justify-evenly items-center gap-5 border-solid border-black border-2 rounded-lg bg-white">
            <BiSearchAlt className="w-7 h-7 ml-3 bg-white" />
          ...
          <div className="flex flex-row justify-evenly items-end h-20 w-100 gap-8">
            <div>
              ...
              <button
                ...
                className="h-10 w-40 m-30 rounded-lg text-base bg-white hover:bg-gray-200 flex flex-row justify-center items-center text-center border-solid border-black border-2"
              >
                {`Ordena por: ${displayName}`}
                <svg
            ...
            {/* BUTTONS: DELETE ICONS */}
            <button
              className="h-10 w-40 m-30 rounded-lg bg-white hover:bg-gray-200 font-bold border-solid border-black border-2"
        ...
        <div className="p-3 flex flex-col gap-8 justify-between w-auto items-center h-auto p-6 pl-12 pr-12 bg-white rounded-lg sm:flex-col md:flex-col lg:flex-row">
          {/* ICONS */}
          <div className="flex flex-wrap flex-row lg:flex-nowrap justify-center items-center gap-5 ">
            {iconImages.map((icon) => (
              <div
                key={icon.iconFilterValue}
                className={`w-2/5 md:w-1/6 border-solid hover:cursor-pointer hover:shadow-xl hover:scale(1.2 : 1)`}
                onClick={(): any =>
                  iconSort({ iconFilterValue: icon.iconFilterValue })
                }
              >
                <Image
                  src={icon.url}
                  alt={icon.iconFilterValue}
                  style={{
                    transform: `scale(${
                      clickedIcon === icon.iconFilterValue ? 0.8 : 1
                    })`,
                    border: `${
                      clickedIcon === icon.iconFilterValue
                        ? "solid black"
                        : "none"
                    }`,
                  }}
          ...
          <div className="flex flex-row justify-center items-center h-20 w-100 gap-4 sm:flex-row md:flex-row lg:gap-8 lg:border-l-4 border-grey pl-4">
            {/* BUTTONS: MORE FILTERS */}
            <button
              className="h-10 w-40 m-30 rounded-lg border-solid border-black border-2 hover:bg-gray-200"
              onClick={() => showMoreFiltersHandler()}
            >
              Más Filtros:
            </button>

            {/* BUTTONS: MAP */}
            <button className="h-10 w-40 m-30 rounded-lg border-solid border-black border-2 hover:bg-gray-200">
              Ver Mapa
            </button>
          </div>
        </div>
